...
function setup() {
	...
	walls.push(
		new Wall(MAP_X, MAP_Y, MAP_X, MAP_Y + MAP_HEIGHT, color(200, 150), true)
	);
	walls.push(
		new Wall(MAP_X, MAP_Y, MAP_X + MAP_WIDTH, MAP_Y, color(200, 150), true)
	);
	walls.push(
		new Wall(
			MAP_X,
	...
	if (SHOW_MAP) {
		if (pressing) {
			...
			if (mode == 'line') {
				if (
					pressEnd.x > MAP_X &&
					pressEnd.x < MAP_X + MAP_WIDTH &&
					pressEnd.y > MAP_Y &&
					pressEnd.y < MAP_Y + MAP_HEIGHT
				) {
...
function mousePressed() {
	if (SHOW_MAP) {
		...
		if (
			pressStart.x > MAP_X &&
			pressStart.x < MAP_X + MAP_WIDTH &&
			pressStart.y > MAP_Y &&
			pressStart.y < MAP_Y + MAP_HEIGHT
		) {
			if (mode == 'line') {
				walls.push(
					new Wall(
